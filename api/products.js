...
apiRouter.post('/create', async (req, res) => {
  try {
    ...
    const productToCreate = await Products.createProduct({title, description, price, image});
...
apiRouter.patch('/update', async (req, res) => {
  ...
  try {
    ...
    const productToUpdate = await Products.updateProduct(id, {title: title, description: description, price: price, image: image});
